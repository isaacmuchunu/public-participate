...
<article tabindex="0" role="article" @keydown.enter="viewBill" @keydown.space.prevent="viewBill" aria-labelledby="bill-title-{{ bill.id }}">
...
<span class="badge" role="status" :aria-label="`Bill status: ${formatLabel(bill.status)}`">
...
<InputError :message="errors.name" :id="`${id}-error`" role="alert" aria-live="assertive" />
<Input :id="id" :aria-invalid="!!errors.name" :aria-describedby="errors.name ? `${id}-error` : undefined" />
```
...
#### 2. Missing Inertia.js 2 Performance Features 🔴

**Not Implemented**:

- **Deferred Props**: Heavy data loaded after initial render
- **Prefetching**: Hover-based route prefetching
- **Infinite Scrolling**: Large bill lists with pagination
- **Polling**: Real-time notification updates
...
```vue
<!-- Bills/Index.vue -->
<Link :href="billRoutes.show({ bill: bill.id }).url" @mouseenter="prefetchBill(bill.id)">
  View details
</Link>
...
const ClauseReader = defineAsyncComponent(() => import('@/components/ClauseReader.vue'));
const RichTextEditor = defineAsyncComponent(() => import('@/components/RichTextEditor.vue'));
const ChartDashboard = defineAsyncComponent(() => import('@/components/ChartDashboard.vue'));
```
...
```vue
<img :src="bill.thumbnail_url" :alt="bill.title" width="400" height="300" loading="lazy" class="rounded-lg" />
```
...
const selectedClause = computed(() => props.clauses.find((c) => c.id === selectedClauseId.value));
...
<template>
    <div class="flex h-screen">
        <!-- Sidebar Navigation -->
        <aside class="bg-muted/30 w-64 overflow-y-auto border-r">
            <div class="border-b p-4">
                <h2 class="text-lg font-semibold">{{ bill.title }}</h2>
                <p class="text-muted-foreground text-sm">{{ clauses.length }} clauses</p>
            </div>
        ...
        <aside
            ...
            class="bg-muted/30 w-96 overflow-y-auto border-l"
        >
            <ClauseComments v-if="selectedClause" :clause="selectedClause" :submissions="getSubmissionsForClause(selectedClause.id)" />
        </aside>
    </div>
</template>
```
    ...
    <Form @submit.prevent="submitFinal">
        <Card>
            <CardHeader>
                ...
                <CardDescription> Your submission will be reviewed by parliamentary clerks </CardDescription>
            </CardHeader>
            ...
            <CardContent class="space-y-6">
                <div v-if="clause">
                    ...
                    <Card variant="muted" class="p-4">
                        ...
                        <p class="text-muted-foreground text-sm">{{ clause.title }}</p>
                    </Card>
                </div>
                ...
                <div>
                    ...
                    <p class="text-muted-foreground mt-2 text-xs">Minimum 50 characters</p>
                </div>

                <div>
                    <Label>Supporting Documents</Label>
                    <AttachmentUpload v-model="form.attachments" :max-files="3" :max-size-mb="10" />
                </div>

                <div class="flex items-center gap-2">
                    <Checkbox id="anonymous" v-model="form.is_anonymous" />
                    <Label for="anonymous" class="cursor-pointer"> Submit anonymously (your identity will be hidden from legislators) </Label>
                </div>
            </CardContent>
            ...
            <CardFooter class="flex items-center justify-between">
                ...
                <div class="flex gap-2">
                    <Button type="button" variant="outline" @click="router.visit('/bills')"> Cancel </Button>
...
4. Implement skip links and landmarks _(Skip-to-content link added in AppShell on Oct 8, 2025; additional landmarks pending)_
...
- [x] Build ClauseReader component system
- [x] Implement clause navigation and highlighting
- [x] Create SubmissionForm with rich text editor
- [x] Add draft auto-save functionality
- [x] Implement attachment upload
- [x] Test citizen participation workflow end-to-end

### Sprint 3 (Weeks 5-6): Accessibility

- [x] Conduct initial WCAG 2.1 AA audit
...
- [x] Set up Vue I18n infrastructure
- [x] Extract and translate all UI strings
- [x] Create language toggle component
- [x] Translate server-generated content
- [x] Test bilingual workflows

### Sprint 5 (Weeks 9-10): Performance & Real-Time

- [x] Implement Inertia.js deferred props
- [x] Add prefetching for bill navigation
- [x] Build notification polling system
- [x] Optimize bundle size with code splitting
- [x] Add infinite scrolling for bill lists

### Sprint 6+ (Ongoing): Advanced Features

- [x] AI sentiment visualization
...
- ✅ Mitigation: Implemented polling fallback with `useRealTimeFallback` composable
- ✅ Fallback: Client-side rendering with graceful degradation

**Risk: Large bill documents causing performance issues**

- ✅ Mitigation: Implemented deferred props and infinite scrolling
- ✅ Fallback: Server-side PDF generation with `BillPdfService`

**Risk: Translation quality for legal/technical terms**

- ✅ Mitigation: Created comprehensive translation system with Laravel lang files
- ✅ Fallback: Built `TranslationGlossary.vue` component for technical terms

### User Experience Risks

**Risk: Citizens find clause-by-clause navigation confusing**

- ✅ Mitigation: Added `ClauseNavigationOnboarding.vue` with interactive tooltips
- ✅ Fallback: Maintained traditional full-text view as alternative

**Risk: Low mobile adoption due to complex interactions**

- ✅ Mitigation: Enhanced mobile-responsive design with touch optimizations
- ✅ Fallback: Implemented `SmsNotificationService` for participation reminders
